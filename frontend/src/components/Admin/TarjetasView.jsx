...
export default function TarjetasView() {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditing ? `${import.meta.env.VITE_API_URL}/api/servicios/${formData.id}/` : `${import.meta.env.VITE_API_URL}/api/servicios/`;
    ...
    <div className={styles.section}>
          ...
          <div className={styles.modalContent}>
            ...
            <form className={styles.form} onSubmit={handleSubmit}>
              <div className={styles.formGrid}>
                ...
                <div className={styles.formGroup}>
                  <label>Estado</label>
                  <select value={formData.activa} onChange={e => setFormData({...formData, activa: e.target.value === 'true'})}>
                ...
                <div className={styles.formGroupFull}>
                  <label>Imagen</label>
                  <input type="file" accept="image/*" onChange={e => setImagenTarjeta(e.target.files[0])} />
                </div>
              </div>
              <div className={styles.modalActions}>
                <button type="button" onClick={resetForm} className={styles.cancelBtn}>Cancelar</button>
                <button type="submit" className={styles.submitBtn}>Guardar Tarjeta</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className={styles.productsList}>
        <table className={styles.table}>
           {/* ... Estructura de tabla idéntica a la que ya tenías ... */}
           <thead>
           ...
           <tbody>
             {tarjetas.map(t => (
               <tr key={t.id}>
                 <td>{t.id}</td><td>{t.nombre || t.titulo}</td><td>{t.activo ? 'Pública' : 'Oculta'}</td>
                 <td>
